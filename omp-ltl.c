/****************************************************************************
 *
 * omp-ltl.c - Larger than Life OpenMP version
 *
 * Written by Martina Cavallucci <martina.cavallucci(at)studio.unibo.it>
 * --------------------------------------------------------------------------
 *
 * OpenMP computation of the Larger Than Life
 * Compile with:
 *
 * gcc -fopenmp -std=c99 -Wall -Wpedantic omp-ltl.c -o omp-ltl
 *
 * Run with:
 * (Test it with the Game of Life parameter )
 * ./omp-ltl (R =) 1 (B1 = B2 =) 3 (D1 =) 3  (D2 = )4 nsteps input_file output_file
 *
 ****************************************************************************/
 #include "hpc.h"
 #include <stdio.h>
...
/*Count how many neighbors are living in the range of i j element */
 ...
 {
   int nbors = 0;
   #pragma omp parallel for reduction(+:nbors) collapse(2)
   for(int k = 1 ; k < r + 1 ; k++){
     for(int f= 1 ; f < r + 1 ; f++){
          nbors = nbors +
 ...
 {
    int i, j, element, c = 0;
    #pragma omp parallel for private(c,element) collapse(2)
   	 for (i = r; i < n + r ; i++)
   	  {
       for (j = r; j < n + r; j++)
        {
